...
export async function getArticles(
  ...
  try {
    const url = `${STRAPI_URL}/api/articles?fields[0]=title&fields[1]=slug&fields[2]=excerpt&fields[3]=publishedAt&populate[coverImage][fields][0]=url&pagination[page]=${page}&pagination[pageSize]=${pageSize}&sort[0]=publishedAt:desc&filters[site][slug][$eq]=${SITE_FILTER}`;
...
export async function getArticle(slug: string): Promise<Article | null> {
  try {
    const url = `${STRAPI_URL}/api/articles?filters[slug][$eq]=${slug}&filters[site][slug][$eq]=${SITE_FILTER}&populate=*`;
...
export async function getTags(): Promise<Tag[]> {
  try {
    const url = `${STRAPI_URL}/api/tags?filters[articles][site][slug][$eq]=${SITE_FILTER}&fields[0]=name&fields[1]=slug&pagination[pageSize]=100`;
...
export async function getArticlesByTag(
  ...
  try {
    ...
    const url = `${STRAPI_URL}/api/articles?fields[0]=title&fields[1]=slug&fields[2]=excerpt&fields[3]=publishedAt&populate[coverImage][fields][0]=url&pagination[page]=${page}&pagination[pageSize]=${pageSize}&sort[0]=publishedAt:desc&filters[site][slug][$eq]=${SITE_FILTER}&filters[tags][slug][$eq]=${tagSlug}`;
